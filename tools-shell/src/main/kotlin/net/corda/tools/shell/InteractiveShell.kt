...
import com.sun.prism.shader.Texture_RadialGradient_REFLECT_Loader
...
object InteractiveShell {
    ...
    fun startShell(configuration: ShellConfiguration, classLoader: ClassLoader? = null) {
        log.info("binhnt.InteractiveShell.startShell :  create rpcOps: $configuration ")
        ...
        log.info("binhnt.InteractiveShell.startShell :  call  _startShell: $classLoader")
    ...
    private fun _startShell(configuration: ShellConfiguration, classLoader: ClassLoader? = null) {
        log.info("binhnt.InteractiveShell._startShell :  start shell ")
        ...
        if (runSshDaemon) {
            log.info("binhnt.InteractiveShell._startShell :  run runSshDaemon ")
            ...
            log.info("binhnt.InteractiveShell._startShell :  port: " + configuration.sshdPort?.toString())
        ...
        log.info("binhnt.InteractiveShell._startShell :  start ShellLifecycle: config = $config , configuration:  $configuration ")
        shell = ShellLifecycle(configuration.commandsDirectory).start(config, configuration.user, configuration.password)
        if(shell == null){
            log.info("binhnt.InteractiveShell._startShell :  shell is null")
    ...
    fun runLocalShell(onExit: () -> Unit = {}) {
        ...
        log.info("binhnt.InteractiveShell.runLocalShell :  call node.stop when existed")
        this.onExit = onExit

        log.info("binhnt.InteractiveShell.runLocalShell :  create terminal by TerminalFactory.create() ")

        try {
           val terminal = TerminalFactory.create()
            log.info("binhnt.InteractiveShell.runLocalShell :  terminal created ")

            log.info("binhnt.InteractiveShell.runLocalShell :  create consoleReader ")
            val consoleReader = ConsoleReader("Corda", FileInputStream(FileDescriptor.`in`), System.out, terminal)

            log.info("binhnt.InteractiveShell.runLocalShell :  consoleReader: $consoleReader ")

            log.info("binhnt.InteractiveShell.runLocalShell :  create jlineProcessor ")
            val jlineProcessor = JLineProcessor(terminal.isAnsiSupported, shell, consoleReader, System.out)
            log.info("binhnt.InteractiveShell.runLocalShell :  jlineProcessor: $jlineProcessor ")

            InterruptHandler { jlineProcessor.interrupt() }.install()

            log.info("binhnt.InteractiveShell.runLocalShell :  start thread for jlineProcessor.run() ")
            ...
            log.info("binhnt.InteractiveShell.runLocalShell :  start thread for Command line shell terminator ")
            ...
            log.info("binhnt.InteractiveShell.runLocalShell : Error: $ex")
        }
    ...
    class ShellLifecycle(private val shellCommands: Path) : PluginLifeCycle() {
        fun start(config: Properties, localUserName: String = "", localUserPassword: String = ""): Shell {
           ...
           // log.info("$config")
            log.info("binhnt: InteractiveShell.ShellLifecycle .start :  start: user:  "+ localUserName + ", password: " + localUserPassword)
            ...
            log.info("binhnt: InteractiveShell.ShellLifecycle: $commandsFS and $confFS ")
            ...
            log.info("binhnt: InteractiveShell.ShellLifecycle: context= $context ")
            context.refresh()
            this.config = config
            log.info("binhnt.ShellLifecycle :  start context: $config")
            start(context)

            log.info("binhnt.ShellLifecycle.start :  call makeRPCOps: $rpcOps, $localUserName, $localUserPassword")
            ops = makeRPCOps(rpcOps, localUserName, localUserPassword)

            log.info("binhnt.ShellLifecycle.start :  return shell plugin with ShellFactory::class")
            val shellplugin = context.getPlugin(ShellFactory::class.java)
                .create(null, CordaSSHAuthInfo(false, ops, StdoutANSIProgressRenderer))

            if (shellplugin == null){
                log.info("binhnt.ShellLifecycle.start :  Error create shell plugin." )
            }
            return shellplugin
        }
    }
